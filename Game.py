...
pygame.mixer.init() #Initiazation of Sound
# Sound list
paddle_sound = pygame.mixer.Sound('sounds/Paddle.wav')
game_over_sound = pygame.mixer.Sound('sounds/Restart.wav')
    ...
    if ball.colliderect(player):
        if abs(ball.bottom - player.top) < 10:  # Check if ball hits the top of the paddle
            ...
            def score_increase(score):
                return score +1
            score = score_increase(score) # Increase player score
            paddle_sound.play() # Play paddle collision sound
            ball_speed_y *= -1  # Reverse ball's vertical direction
            # TODO Task 3: Increase the ball's speed by x
            def speed_increase(ball_speed_x):
                return ball_speed_x * 1.05
            ball_speed_x = speed_increase(ball_speed_x)
            ball_speed_y = speed_increase(ball_speed_y)
...
def restart():
    ...
    game_over_sound.play() #Play restart sound
...
while True:
    ...
    pygame.draw.rect(screen, lavender, player)  # Draw player paddle
    # TODO Task 1: Change color of the ball
    pygame.draw.ellipse(screen, purple_lavender, ball)  # Draw ball
    player_text = basic_font.render(f'{score}', False, turquoise)  # Render player score
